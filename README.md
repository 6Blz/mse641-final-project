...
| Version | What changed | Model | Kaggle |
|---|---|---|---|
| `task2_v1.py` | one span, same length cap for every type | roberta-base-squad2 | 0.45647 |
| `task2_v2.py` | train on every gold segment, add a type classifier to route the decoder | roberta-base-squad2 + roberta-base | 0.45287 |
| `task2_v3.py` | weight segments by `1/n`, average three seeds, tune the decoder with 5-fold CV | roberta-base-squad2 + roberta-base | 0.45133 |
| `task2_v4.py` | grow passage and multi spans out to their sentence, bigger backbone | deberta-v3-large-squad2 + roberta-base | 0.46653 |
